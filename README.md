...
> Built as a production-ready prototype on synthetic data. Every component is architected so it can be swapped for real HOTWORX data sources without rewriting business logic.
...
|---|---|---|---|
| `admin` | `admin123` | Admin | All studios |
| `gm_s01` | `gm123` | GM | Studio S01 only |
| `gm_s02` | `gm123` | GM | Studio S02 only |
| ... | ... | ... | ... |
| `gm_s10` | `gm123` | GM | Studio S10 only |

> Passwords are hashed with PBKDF2-HMAC-SHA256 at runtime. Override defaults via `DASHBOARD_ADMIN_PW` and `DASHBOARD_GM_PW` environment variables.
